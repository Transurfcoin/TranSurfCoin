...
const char * const BITCOIN_CONF_FILENAME = "transurfcoin.conf";
const char * const BITCOIN_PID_FILENAME = "transurfcoin.pid";
...
#else
    const char* pszModule = "transurfcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Transurfcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Transurfcoin";
#else
    // Unix
    return pathRet / ".transurfcoin";
#endif
#endif
}
